#include "gpupixel/custom_filter/healthy_filter.h"
#include "core/gpupixel_context.h"
#include "utils/util.h"

namespace gpupixel {
  const std::string kHealthyFragmentShaderString = R"(
    ...
    void main() {
      ...
      gl_FragColor = vec4(mix(originColor.rgb, c6.rgb, intensity), 1.0);
      // end
    }
  )";

  std::shared_ptr<HealthyFilter> HealthyFilter::Create() {
    auto ret = std::shared_ptr<HealthyFilter>(new HealthyFilter());
  ...
  bool HealthyFilter::Init() {
    if (!InitWithFragmentShaderString(kHealthyFragmentShaderString)) {
      return false;
    }

    auto path = Util::GetResourcePath() / "res";
    mask_image_ = SourceImage::Create((path / "healthy_mask_1.jpg").string());
    curve_image_ = SourceImage::Create((path / "lookup_healthy.png").string());
    return true;
  }

  bool HealthyFilter::DoRender(bool updateSinks) {
    ...
    glBindTexture(GL_TEXTURE_2D, mask_image_->GetFramebuffer()->GetTexture());
    filter_program_->SetUniformValue("mask", 4);
  ...
  void HealthyFilter::setIntensity(float newIntensity) {
    intensity = newIntensity;
  }

  void HealthyFilter::setTexelWidth(int textureWidth) {
    texel_size_x_ = textureWidth;
  }

  void HealthyFilter::setTexelHeight(int textureHeight) {
